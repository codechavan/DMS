CREATE PROCEDURE [dbo].[usp_Get_Systems]
(
	@iPageIndex INT = 0,  
	 @iPageSize INT = 0,  
...
BEGIN
   ...
   SELECT	@iPageIndex = COALESCE( @iPageIndex, 0 ),  
		   	...
		   	@strWhere = COALESCE( @strWhere, 'SystemIsActive = 1' ),  
	...
	SELECT	@iRowStart = ( ( @iPageIndex - 1 ) * ( @iPageSize ) ) + 1,  
			...
			@strOrderBy = ( CASE WHEN @strOrderBy = '' THEN 'SystemName' ELSE @strOrderBy END )  
	
	
	SELECT	@strObject = '[dbo].[Systems]'
