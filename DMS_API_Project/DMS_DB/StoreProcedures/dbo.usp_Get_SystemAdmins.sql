CREATE PROCEDURE [dbo].[usp_Get_SystemAdmins]
(
	@iPageIndex INT = 0,  
	 @iPageSize INT = 0,  
...
BEGIN
   ...
   SELECT	@iPageIndex = COALESCE( @iPageIndex, 0 ),  
		   	...
		   	@strWhere = COALESCE( @strWhere, 'SystemID = 0' ),  
	...
	SELECT	@iRowStart = ( ( @iPageIndex - 1 ) * ( @iPageSize ) ) + 1,  
			...
			@strOrderBy = ( CASE WHEN @strOrderBy = '' THEN 'UserName' ELSE @strOrderBy END )  
	
	
	SELECT	@strObject = '[dbo].[SystemAdmins]'
