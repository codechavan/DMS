CREATE PROCEDURE [dbo].[usp_Get_Configurations]
(
	@iPageIndex INT = 0,  
	 @iPageSize INT = 0,  
...
BEGIN
   ...
   SELECT	@iPageIndex = COALESCE( @iPageIndex, 0 ),  
	...
	SELECT	@iRowStart = ( ( @iPageIndex - 1 ) * ( @iPageSize ) ) + 1,  
			...
			@strOrderBy = ( CASE WHEN @strOrderBy = '' THEN '[ConfigurationCode]' ELSE @strOrderBy END )  
	
	
	SELECT	@strObject = '[dbo].[vw_Configurations]'
